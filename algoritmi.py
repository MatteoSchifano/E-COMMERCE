...
def preprocessing(df):
    ...
    # df['prezzo'] = scaler.fit_transform(df['prezzo'].array.reshape(-1,1))
...
def prodotti_correlati(df, _id_acquisto):
    _, indices = predicta(df)
    prodotto_acquistato = df.loc[df['_id'] == _id_acquisto]
    print(prodotto_acquistato.index[0], type(prodotto_acquistato))

    for x in indices:
        if x[0] == prodotto_acquistato.index[0]:
            ls = [df.iloc[y, 0] for y in x[1:]]

    return ls

def consiglia_prodotti(carrello):
    ...
    ls_prodotti_correlati = prodotti_correlati(df, carrello)
